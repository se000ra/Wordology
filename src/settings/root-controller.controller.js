class RootController {
  constructor ($scope) {
    ...
    this.OM.getGlobalOptions().then(options => {
  ...
  async reloadProfiles () {
    ...
    if (profileId !== null) {
      this.selectedProfile = this.profiles.filter(
  ...
  async onClickRenameProfile () {
    var name = await this.showPrompt(
      this.strings.RENAME_PROFILE(this.selectedProfile.name)
    )
    if (name) {
  ...
  async onClickDeleteProfile () {
    var response = await this.showPrompt(
      this.strings.DELETE_PROFILE_WARNING(this.selectedProfile.name)
    )
