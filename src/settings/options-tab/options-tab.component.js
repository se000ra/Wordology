class OptionsTabController {
  ...
  addPrefix () {
    ...
    setTimeout(() => {
      ...
      this.OM.setOption(
        'blacklistedPrefixes',
        this.blacklistedPrefixes.map(row => row.prefix)
      )
      this.$scope.$digest()
    })
  }
  ...
  async reloadOptions () {
    try {
      ...
      setTimeout(() => {
        this.blacklistedPrefixes = this.options.blacklistedPrefixes.map(
          prefix => ({ prefix: prefix })
        )
        this.whitelistedSuffixes = this.options.whitelistedSuffixes.map(
  ...
  onDeletePrefix (deletedPrefix) {
    var newList = this.blacklistedPrefixes
      .map(row => row.prefix)
      .filter(prefix => prefix !== deletedPrefix)
    this.OM.setOption('blacklistedPrefixes', newList)
  }

  onDeleteSuffix (deletedSuffix) {
    var newList = this.whitelistedSuffixes
      .map(row => row.suffix)
      .filter(suffix => suffix !== deletedSuffix)
  ...
  addSuffix () {
    ...
    setTimeout(() => {
      ...
      this.OM.setOption(
        'whitelistedSuffixes',
        this.whitelistedSuffixes.map(row => row.suffix)
      )
      this.$scope.$digest()
    })
  }
