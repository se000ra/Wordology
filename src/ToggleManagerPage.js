class ToggleManagerPage {
  constructor (name, config) {
    ...
    new MessageSlot(name + 'ToggleFirstOn', async (message, sender) => {
      await this.onFirstOn()
      this.doneRunningMessageSender.sendToRuntime({})
    })

    new MessageSlot(name + 'ToggleOn', async (message, sender) => {
      await this.onToggleOn()
      this.doneRunningMessageSender.sendToRuntime({})
    })

    new MessageSlot(name + 'ToggleOff', (message, sender) => this.onToggleOff())

    new MessageSlot(name + 'LoggedOut', (message, sender) => this.onLoggedOut())

    new MessageSlot(name + 'LoggedOutPress', (message, sender) =>
      this.onLoggedOutPress()
    )

    new MessageSlot(name + 'ChangedProfile', (message, sender) =>
      this.onChangedProfile()
    )

    new MessageSlot(name + 'ChangedProfilePress', (message, sender) =>
      this.onChangedProfilePress()
    )
