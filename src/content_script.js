;(function ContentScript () {
  ...
  async function init () {
    ...
    document
      ...
      .style.setProperty(
        ...
        cssStringFromHex(
          options.notDefinedColor,
          options.notDefinedOpacity / 100
        )
      )
    document
      .querySelector(':root')
  ...
  async function onClickWord (e) {
    ...
    if (window.getSelection().toString().length === 0) {
      ...
      if (userResponse.definition) {
        ...
        var entry = {
          word: wordElement.getWord(),
          definition: userResponse.definition
        }
        DF.setData([entry])
      }
    }
  ...
  async function showLoggedOutPopup () {
    const options = await new OptionsManager().getGlobalOptions()
