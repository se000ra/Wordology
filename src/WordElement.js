...
class WordElement {
  constructor (dom, text) {
    ...
    this.span.setAttribute(
      WordElement.ATTRIBUTE_NAME,
      WordElement.ATTRIBUTE_UNDEFINED
    )
    ...
    this.span.addEventListener(
      ...
      function onMouseOver (e) {
        if (!this.hidden) {
          this.mouseOver.dispatchEvent({ target: this, domEvent: e })
        }
      }.bind(this)
    )
    this.span.addEventListener(
      ...
      function onMouseOut (e) {
        if (!this.hidden) {
          this.mouseOut.dispatchEvent({ target: this, domEvent: e })
        }
      }.bind(this)
    )
    this.span.addEventListener(
      ...
      function onClick (e) {
        if (!this.hidden) {
          this.click.dispatchEvent({ target: this, domEvent: e })
        }
      }.bind(this)
    )
  }
  ...
  show () {
    if (this.match) {
      if (this.hasExactMatch()) {
        this.span.setAttribute(
          WordElement.ATTRIBUTE_NAME,
          WordElement.ATTRIBUTE_DEFINED
        )
      } else {
        this.span.setAttribute(
          WordElement.ATTRIBUTE_NAME,
          WordElement.ATTRIBUTE_SIMILAR
        )
      }
    } else {
      this.span.setAttribute(
  ...
  showPopup () {
    ...
    for (const [key, cssClass] of Object.entries(WordElement.CSSClasses)) {
