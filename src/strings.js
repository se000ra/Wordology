const WordologyStrings = {
  strings: {
    ...
    LOOKUP_DESCRIPTION: {
      en:
        'Enter the URL of your preferred online dictionary, typing a dollar sign ($) where the word should be, e.g.: <em>https://en.wiktionary.org/wiki/$</em>.',
      fr:
        "Entrez l'URL de votre dictionnaire en-ligne préferé, tapant un signe dollar ($) là où devrait figurer le mot, e.g.: <em>https://fr.wiktionary.org/wiki/$</em>.",
      pl:
        'Wpisz tutaj URL twojego preferowanego internetowego słownika, dodając znak dolara ($) tam gdzie powinno być słowo, n.p.: <em>https://pl.wiktionary.org/wiki/$</em>.',
      ru:
        'Введите URL предпочитаемого вами онлайн-словаря и введите знак доллара ($) там, где должно быть слово, например: <em>https://ru.wiktionary.org/wiki/$</em>.',
      es:
        'Ingrese la URL de su diccionario en línea preferido, escribiendo un signo de dólar ($) donde debería estar la palabra, por ejemplo: <em>https://es.wiktionary.org/wiki/$</em>.',
      pt:
        'Digite o URL do seu dicionário online preferido, digitando um símbolo de dólar ($) onde a palavra deve estar, por exemplo: <em>https://pt.wiktionary.org/wiki/$</em>.'
    },
    LOOKUP_URL: {
      en: 'Lookup URL:',
      fr: 'URL de recherche:',
    ...
    TRANSLATION_SUGGESTED: {
      en: word =>
        `Translation suggested based on similarity to <em>${word}</em>.`,
      fr: word =>
        `Traduction suggérée dû à la similarité avec <em>${word}</em>.`,
      pl: word =>
        `Przetłumaczenie zaproponowany na postawie podobieństwa z <em>${word}</em>.`,
      ru: word =>
        `Предлагаемый перевод основан на сходстве с <em>${word}</em>.`,
      es: word =>
        `Traducción sugerida basada en similitud con <em>${word}</em>.`,
      pt: word =>
        `Tradução sugerida com base na semelhança com <em>${word}</em>.`
    },
    LOGGED_OUT: {
      en:
        'You don\'t seem to be logged into a Wordology profile right now. Go to your settings page to log in, and then <a href="javascript:window.location.reload();">reload</a> this page.',
      fr:
        'Vous ne semblez pas être connecté à un profil Wordology. Allez à la page de paramètres pour vous connecter, puis <a href="javascript:window.location.reload();">rechargez</a> cette page.',
      pl:
        'Wydaje się, że nie jesteś obecnie zalogowany na profil Wordology. Przejdź do strony ustawienia aby się zalogować, a potem <a href="javascript:window.location.reload();">załaduj</a> tę stronę.',
      ru:
        'Похоже, вы не вошли сейчас в профиль Wordology. Перейдите на страницу настроек, чтобы войти в систему, а затем <a href="javascript:window.location.reload();">перезагрузите</a> эту страницу.',
      es:
        'No parece haber iniciado sesión en un perfil de Wordology en este momento. Vaya a su página de configuración para iniciar sesión y luego <a href="javascript:window.location.reload();">recargar</a> esta página.',
      pt:
        'Você não parece estar conectado a um perfil do Wordology no momento. Vá para a página de configurações para efetuar login e, em seguida, <a href="javascript:window.location.reload();">recarregue</a> esta página.'
    },
    CHANGED_PROFILE: {
      en:
        'You seem to have changed Wordology profiles since the last time this page was loaded. <a href="javascript:window.location.reload();">Reload</a> this page to keep using Wordology.',
      fr:
        'Vous semblez avoir changé de profil Wordology depuis que cette page ait été chargée. <a href="javascript:window.location.reload();">Rechargez</a> cette page pour continuer à utiliser Wordology.',
      pl:
        'Wydaje się, że zmieniłeś profil Wordology od kiedy była ładowana ta strona. <a href="javascript:window.location.reload();">Załaduj</a> tę stronę aby nadal używać Wordology.',
      ru:
        'Похоже, вы изменили профили Wordology с момента последней загрузки этой страницы. <a href="javascript:window.location.reload();">Перезагрузите</a> эту страницу, чтобы продолжать использовать Wordology.',
      es:
        'Parece que ha cambiado de perfil de Wordology desde la última vez que se cargó esta página. <a href="javascript:window.location.reload();">Recarga</a> esta página para seguir usando Wordology.',
      pt:
        'Você parece ter alterado os perfis do Wordology desde a última vez que esta página foi carregada. <a href="javascript:window.location.reload();">Recarregue</a> esta página para continuar usando o Wordology.'
    },
    WORDS: {
      en: 'Words',
      fr: 'Mots',
    ...
    EXTENDED_ALPHABET_DESCRIPTION: {
      ...
      fr:
        'Ajoutez des caractères supplémentaires à l\'alphabet reconnu par Wordology. Par exemple, en ajoutant le tiret ("-"), Wordology considérera "tire-bouchon" comme un seul mot plutôt que "tire" et "bouchon". Ecrivez simplement tous les caractères à la suite, sans éspaces, e.g. <em>\'-</em> pour ajouter \' et -.',
      ...
      es:
        'Agregue caracteres adicionales al alfabeto reconocido por Wordology. Por ejemplo, si agrega el guión ("-"), Wordology considerará "norte-sur" como una palabra en lugar de "norte" y "sur". Simplemente escriba todos los caracteres adicionales juntos, sin espacios, p.ej. <em>\'-</em> para agregar \' y -.',
      pt:
        'Adicione caracteres extras ao alfabeto reconhecido por Wordology. Por exemplo, se você adicionar o hífen ("-"), Wordology considerará "norte-americano" como uma palavra em vez de "norte" e "americano". Basta digitar todos os caracteres extras juntos, sem espaços, por exemplo <em>\'-</em> para adicionar \' e -.'
    },
    ...
    IMPORT_ERROR: {
      ...
      fr:
        'Une erreur est survenue en lisant votre fichier. Etait-ce le bon fichier?',
      ...
      es:
        'Se produjo un error al leer su archivo. ¿Era ese el archivo correcto?',
      pt: 'Ocorreu um erro ao ler seu arquivo. Esse foi o arquivo certo?'
    },
    DELETE_WORDLIST_WARNING: {
      en:
        'To delete your entire wordlist for this profile permanently, type DELETE below in all-capital letters, and click OK.',
      fr:
        'Pour supprimer tous les mots de ce profil de façon permanante, tapez SUPPRIMER ci-dessous en lettre majuscules, et appuyez sur OK.',
      pl:
        'Aby usunąć trwale wszystkie słowa z tego profilu, wpisz poniżej USUNAC dużymi literami i kliknij OK.',
      ru:
        'Чтобы окончательно удалить весь список слов для этого профиля, введите УДАЛИТЬ ниже заглавными буквами и нажмите ОК.',
      es:
        'Para eliminar toda su lista de palabras para este perfil de forma permanente, escriba ELIMINAR en mayúsculas y clique en Aceptar.',
      pt:
        'Para apagar permanentemente sua lista de palavras inteira desse perfil, digite APAGAR abaixo em letras maiúsculas e clique em OK.'
    },
    DELETE_PASSWORD: {
      en: 'DELETE',
      fr: 'SUPPRIMER',
  ...
  getStrings: function (languageCode) {
    const result = {}
    for (const stringName in WordologyStrings.strings) {
