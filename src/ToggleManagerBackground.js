class ToggleManagerBackground {
  ...
  constructor (name, config) {
    ...
    this.loggedOutPressMessageSender = new MessageSender(
      name + 'LoggedOutPress'
    )
    this.changedProfileMessageSender = new MessageSender(
      name + 'ChangedProfile'
    )
    this.changedProfilePressMessageSender = new MessageSender(
      name + 'ChangedProfilePress'
    )

    new MessageSlot(name + 'Ready', (message, sender) =>
      this.onReadyMessage(sender.tab.id)
    )

    new MessageSlot(name + 'DoneRunning', (message, sender) =>
      this.onDoneRunningMessage(sender.tab.id)
    )

    browser.browserAction.onClicked.addListener(tab => this.toggle(tab.id))
  ...
  loggedOut () {
    ...
    for (const tabId of this.tabStateMap.keys()) {
